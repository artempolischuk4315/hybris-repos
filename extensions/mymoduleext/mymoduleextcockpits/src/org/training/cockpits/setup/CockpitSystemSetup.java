...
import org.training.cockpits.constants.MymoduleextCockpitsConstants;
 ...
 */
@SystemSetup(extension = MymoduleextCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(MymoduleextCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(MymoduleextCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						MymoduleextCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/mymoduleextcockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
